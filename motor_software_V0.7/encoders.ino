...
void readElbowEncoder()
{
   if(initElbow == 1)
    ...
    #ifdef MAXIMUM_LIMIT
   else if(elbowEncoder > ELBOW_MAX_POS && !movingForward)
    ...
    else if(elbowDirection == RIGHT)
    {
       ...
       elbowEncoder++;
        #ifdef FOLLOW_YAW
         if(yawFollowElbow == true)
          moveToPos(&yawTarget,(YAW_MAX_POS/2) + yawCounter , &yawEncoder, YAW_PLUS, YAW_MIN, YAW_ENABLE, 65);
        #endif
         if(elbowEncoder %3 == 0)
           yawCounter++;
      #else
    ...
    else if(elbowDirection == LEFT)
    {
      ...
      elbowEncoder--;
        ...
        if(yawFollowElbow == true)
        #endif
         moveToPos(&yawTarget,(YAW_MAX_POS/2) + yawCounter , &yawEncoder, YAW_PLUS, YAW_MIN, YAW_ENABLE, 70);//60
        if(elbowEncoder % 3 == 0)
          yawCounter--;
      #else
      if(elbowEncoder > 0)
...
void readZedEncoder()
{
  if(initZed == 1)
   ...
   // #ifdef MIXIMUM_LIMIT
    else if((zedEncoder >= ZED_MAX_POS && zedDirection != LEFT) ||(zedEncoder <= 100 && zedDirection != RIGHT ))//&& zedDirection != 0
    {
      wasStopped = true;
      stopMotor(ZED_PLUS, ZED_MIN, ZED_ENABLE);
      //Serial.println("zed max");
    }
    //#endif
